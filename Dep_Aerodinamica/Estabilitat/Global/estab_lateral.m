...
% TAIL PARAMETERS
h_v = 5;  %distància vertical entre cg i ac cua vertical
...
ct = 1.3 ; %Tip chord
b = 22; %wingspan
zwb = 1.1; %height of the wing with respect to cg
...
% Aproximación de la pendiente de la curva de sustentación del estabilizador
CL_alpha_cua = (2 * pi) / (1 + (2 / AR_t)); 

% Contribución del estabilizador vertical
Cl_beta_vertical = -eta_v * (Sec_t/Sec) * (h_v/b) * CL_alpha_cua;

% Contribución de la ala
Cl_beta_ala = -a_wb(1)*zwb/b;

% Cálculo total de Cn_beta
Cl_beta_total = Cl_beta_vertical + Cl_beta_ala

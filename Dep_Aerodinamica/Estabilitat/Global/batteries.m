...
v_cruise = 600 / 3.6;   % Cruise speed in m/s
h_cruise = 6000;        % Cruise altitude in m
...
while abs(m - m_prev) > tol && iter < max_iter
    ...
    E_cruise_h = P_cruise * t_cruise_h; % Hybrid cruise energy
    ...
    %% Total energies
    E_elec = E_taxi_out + E_to + E_cruise_e + E_land + E_taxi_in; % Electric
    E_hybrid = E_taxi_out + E_to + E_cruise_h + E_land + E_taxi_in; % Hybrid
    E_comb = E_hybrid - E_elec; % Fuel energy
    ...
    % Fuel
    m_comb = SFC * E_comb / (3.6e6); % Fuel mass (using SFC)
    V_comb = m_comb / d_comb;       % Fuel volume

    % Total mass
    m = m0 + m_bat_tot + m_refrigerante + m_comb;
